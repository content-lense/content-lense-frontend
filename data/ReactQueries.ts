...
export const GenericGetItems = async <T extends unknown>(
  entity: string,
  options?: GenericGetItemsOptions
) => {
  const req = await ApiFetch(
    entity +
    (!options || typeof options.queryString === "undefined"
      ? ""
      : options.queryString +
      (options.rangeOperator
        ? `[${options.rangeOperator}]=${options.rangeLowerBoundary}..${options.rangeUpperBoundary}`
        : ""))
  );
