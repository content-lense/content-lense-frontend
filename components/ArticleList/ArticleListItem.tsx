...
import { Avatar, Chip, Divider, IconButton, ListItem, ListItemAvatar, ListItemButton, ListItemText, Stack, Tooltip, Typography } from "@mui/material";
    ...
    {a.complexities &&
            <Stack direction="row" spacing={1}>
              ...
              <Tooltip title="Reading Time in minutes">
                <Chip icon={<AccessTimeIcon fontSize="small"/>} label={a.complexities.find(complex => complex.part === "body")?.readingTimeInMinutes + " min"} />
                </Tooltip>
              }
              {a.complexities.find(complex => complex.part === "body")?.totalWords &&
              <Tooltip title="Length in Words">
                <Chip icon={<StraightenIcon fontSize="small" />} label={a.complexities.find(complex => complex.part === "body")?.totalWords + " words"} />
                </Tooltip>
              }
              {a.complexities.find(complex => complex.part === "body")?.wienerSachtextIndex &&
              <Tooltip title="Wiender Sachtext Index">
                <Chip icon={<SpellcheckIcon fontSize="small" />} label={a.complexities.find(complex => complex.part === "body")?.wienerSachtextIndex} />
                </Tooltip>
              }
            </Stack>
          }
