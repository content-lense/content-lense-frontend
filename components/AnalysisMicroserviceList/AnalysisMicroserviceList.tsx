...
function AnalysisMicroserviceList() {
    const { data, isLoading } = useQuery(["analysis_microservices"], () => GenericGetItems<AnalysisMicroserviceInterface>("/analysis_microservices"));
    if(isLoading || !data){
        return <CircularProgress />
    }
    return ( <List sx={{alignSelf:"stretch"}}>{data.map(a => <><AnalysisMicroserviceListItem service={a} /><Divider /></>)}</List>)
}
